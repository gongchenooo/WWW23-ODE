...
class Model(ABC):
    ...
    def train_Coor(self, data, num_epochs=5, batch_size=None, lr=None, distributed_labels=None):
        # training with server-side coordination strategy
        # training samples have different weights, decided by samle class
        self.optimizer.learning_rate = self.lr if lr is None else lr
        avg_loss_list = []
        for epoch in range(num_epochs):
            tot_loss = []
            for (batched_x, batched_y) in batch_data(data, batch_size, rng=self.rng, shuffle=False):
                input_data = self.process_x(batched_x)
                target_data = self.process_y(batched_y)
                loss = self.optimizer.run_step_Coor(input_data, target_data, distributed_labels) # training with weights
                tot_loss.append(loss)
            avg_loss_list.append(sum(tot_loss) / len(tot_loss))
        #print('Loss:{}',format(avg_loss_list))
        self.optimizer.end_local_updates() # store current parameters in w after local training
        update = np.copy(self.optimizer.w - self.optimizer.w_on_last_update)
        #self.optimizer.update_w()
    ...
    def test(self, eval_data, max_batch_size=None):
        ...
        if max_batch_size is None:
            max_batch_size = len(eval_data['y'])
        max_batch_size = 1000
    ...
    def gradient(self, x, y, layers=-1):
        x, y = self.process_x(x), self.process_y(y)
        return self.optimizer.gradient(x, y, layers)
